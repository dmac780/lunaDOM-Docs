// lunadom/components/carousel/carousel.js
 ...
 * @cssprop  --luna-carousel-bg - Background color
 * @cssprop  --luna-carousel-color - Text color
 * @cssprop  --luna-carousel-border - Border color
 * @cssprop  --luna-carousel-radius - Border radius override
 * @cssprop  --luna-carousel-height - Carousel height
 * @cssprop  --luna-carousel-accent - Accent color
 */
class LunaCarousel extends HTMLElement {
  ...
  get items() {
    return Array.from(this.querySelectorAll('luna-carousel-item'));
  }
  ...
  render() {
    ...
    const slidesHtml = this.items.map((_, index) => {
      return `<div class="slide"><slot name="slide-${index}"></slot></div>`;
    }).join('');

    this.shadowRoot.innerHTML = `
      <style>
        :host {
          --luna-carousel-aspect-ratio: 16 / 9;
          --luna-carousel-bg: #0a0a0a;
          --luna-carousel-radius: 16px;
          --luna-carousel-accent: var(--luna-accent, #2563eb);
          
          display: block;
          position: relative;
          width: 100%;
          max-width: 100%;
          aspect-ratio: var(--luna-carousel-aspect-ratio);
          background: var(--luna-carousel-bg);
          border-radius: var(--luna-carousel-radius);
          overflow: hidden;
        ...
        .nav-btn:hover {
          ...
          border-color: var(--luna-carousel-accent);
        }
        ...
        .dot.active {
          background: var(--luna-carousel-accent);
          width: 24px;
          border-radius: 4px;
          box-shadow: 0 0 10px var(--luna-carousel-accent);
        }
      ...
      <div class="pagination">
        ${this.items.map((_, i) => `<div class="dot ${i === this.current ? 'active' : ''}" data-index="${i}"></div>`).join('')}
      </div>
    `;
...
customElements.define('luna-carousel', LunaCarousel);