// lunadom/components/date-picker/date-picker.js
 ...
 * @cssprop --luna-dp-bg              - Calendar panel background (default: #141414)
 * @cssprop --luna-dp-border          - Panel + trigger border colour (default: #2a2a2a)
 * @cssprop --luna-dp-radius          - Panel border radius (default: 12px)
 * @cssprop --luna-dp-shadow          - Panel box shadow
 * @cssprop --luna-dp-input-bg        - Trigger input background (default: #1a1a1a)
 * @cssprop --luna-dp-input-radius    - Trigger input border radius (default: 8px)
 * @cssprop --luna-dp-input-color     - Trigger input text colour (default: #eee)
 * @cssprop --luna-dp-focus           - Focus ring / active border colour (default: var(--luna-accent, #2563eb))
 * @cssprop --luna-dp-focus-alpha     - Focus ring shadow (default: rgba(37,99,235,.2))
 * @cssprop --luna-dp-label-color     - Label text colour (default: #fff)
 * @cssprop --luna-dp-helptext-color  - Help text colour (default: #666)
 * @cssprop --luna-dp-day-color       - Default day number colour (default: #ccc)
 * @cssprop --luna-dp-day-hover       - Day hover background (default: #2a2a2a)
 * @cssprop --luna-dp-day-muted       - Outside-month day colour (default: #333)
 * @cssprop --luna-dp-selected-bg     - Selected day background (default: var(--luna-accent, #2563eb))
 * @cssprop --luna-dp-selected-color  - Selected day text colour (default: #fff)
 * @cssprop --luna-dp-range-bg        - In-range day background (default: rgba(37,99,235,.15))
 * @cssprop --luna-dp-today-color     - Today indicator colour (default: var(--luna-accent, #2563eb))
 * @cssprop --luna-dp-nav-color       - Nav arrow colour (default: #666)
 * @cssprop --luna-dp-nav-hover       - Nav arrow hover colour (default: #eee)
 * @cssprop --luna-dp-header-color    - Month/year heading colour (default: #fff)
 * @cssprop --luna-dp-weekday-color   - Weekday label colour (default: #444)
 * @cssprop --luna-dp-z               - Panel z-index (default: 1000)
 *
 * Events:
 * @event luna-change - Emitted when the selected value changes.
 *   detail (single): { value: string }
 *   detail (range):  { valueStart: string, valueEnd: string }
 * @event luna-clear  - Emitted when the value is cleared.
 */
class LunaDatePicker extends HTMLElement {
  ...
  _clear(e) {
    ...
    this.removeAttribute('value-end');
    this._rangeHover = null;
    this.dispatchEvent(new CustomEvent('luna-clear', { bubbles: true, composed: true }));
    this._renderTrigger();
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --luna-dp-bg:             #141414;
          --luna-dp-border:         #2a2a2a;
          --luna-dp-radius:         12px;
          --luna-dp-shadow:         0 16px 48px -8px rgba(0,0,0,.8), 0 0 1px rgba(255,255,255,.06);
          --luna-dp-input-bg:       #1a1a1a;
          --luna-dp-input-radius:   8px;
          --luna-dp-input-color:    #eee;
          --luna-dp-focus:          var(--luna-accent, #2563eb);
          --luna-dp-focus-alpha:    rgba(37,99,235,.2);
          --luna-dp-label-color:    #fff;
          --luna-dp-helptext-color: #666;

          --luna-dp-day-color:      #ccc;
          --luna-dp-day-hover:      #2a2a2a;
          --luna-dp-day-muted:      #333;
          --luna-dp-selected-bg:    var(--luna-accent, #2563eb);
          --luna-dp-selected-color: #fff;
          --luna-dp-range-bg:       rgba(37,99,235,.15);
          --luna-dp-today-color:    var(--luna-accent, #2563eb);
          --luna-dp-nav-color:      #666;
          --luna-dp-nav-hover:      #eee;
          --luna-dp-header-color:   #fff;
          --luna-dp-weekday-color:  #444;
          --luna-dp-z:              1000;
        }
        ...
        .dp-label {
          font-size: 0.875rem;
          font-weight: 600;
          color: var(--luna-dp-label-color);
        }

        .dp-help {
          font-size: 0.75rem;
          color: var(--luna-dp-helptext-color);
        }
        ...
        .trigger-wrap {
          ...
          display: flex;
          align-items: center;
          background: var(--luna-dp-input-bg);
          border: 1px solid var(--luna-dp-border);
          border-radius: var(--luna-dp-input-radius);
        ...
        .trigger-wrap:focus-within:not(.disabled) {
          border-color: var(--luna-dp-focus);
          box-shadow: 0 0 0 3px var(--luna-dp-focus-alpha);
        }
        ...
        #panel {
          ...
          left: 0;
          z-index: var(--luna-dp-z);
          width: 280px;
          background: var(--luna-dp-bg);
          border: 1px solid var(--luna-dp-border);
          border-radius: var(--luna-dp-radius);
          box-shadow: var(--luna-dp-shadow);
          padding: 1rem;
        ...
        .month-label {
          font-size: 0.875rem;
          font-weight: 600;
          color: var(--luna-dp-header-color);
        ...
        .nav-btn {
          ...
          justify-content: center;
          border-radius: 6px;
          color: var(--luna-dp-nav-color);
          transition: color 0.15s ease, background 0.15s ease;
        }

        .nav-btn:hover {
          color: var(--luna-dp-nav-hover);
          background: rgba(255,255,255,.06);
        }
        ...
        .weekday {
          ...
          font-size: 0.65rem;
          font-weight: 600;
          color: var(--luna-dp-weekday-color);
        ...
        .day:hover:not(.disabled):not(.selected) {
          background: var(--luna-dp-day-hover);
          color: #fff;
        }

        .day.muted {
          color: var(--luna-dp-day-muted);
        }

        .day.today:not(.selected) {
          color: var(--luna-dp-today-color);
          font-weight: 700;
        }
        ...
        .day.today:not(.selected)::after {
          ...
          height: 3px;
          border-radius: 50%;
          background: var(--luna-dp-today-color);
        }

        .day.selected,
        .day.range-start,
        .day.range-end {
          background: var(--luna-dp-selected-bg);
          color: var(--luna-dp-selected-color);
          font-weight: 600;
          border-radius: 6px;
        }

        .day.in-range {
          background: var(--luna-dp-range-bg);
          border-radius: 0;
          color: var(--luna-dp-day-color);
        }
...
customElements.define('luna-date-picker', LunaDatePicker);