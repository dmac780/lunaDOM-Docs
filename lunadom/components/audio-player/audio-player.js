// lunadom/components/audio-player/audio-player.js

/**
 * @customElement luna-audio-player
 *
 * Attributes:
 ...
 * @cssprop --luna-audio-bg              - Player background (default: #111)
 * @cssprop --luna-audio-fg              - Primary text colour (default: #f0f0f0)
 * @cssprop --luna-audio-accent          - Accent / playhead colour (default: #ff5500)
 * @cssprop --luna-audio-muted           - Secondary / muted text colour (default: #555)
 * @cssprop --luna-audio-waveform        - Inactive waveform bar colour (default: #2a2a2a)
 * @cssprop --luna-audio-waveform-played - Played waveform bar colour (default: var(--luna-audio-accent))
 * @cssprop --luna-audio-radius          - Border radius of the player shell (default: 12px)
 * @cssprop --luna-audio-width           - Max width of the player (default: 480px)
 * @cssprop --luna-audio-padding         - Inner padding (default: 20px 22px 18px)
 *
 * Events:
 * @event luna-play    - Emitted when playback starts. detail: { src, group }
 * @event luna-pause   - Emitted when playback pauses. detail: { src, group }
 * @event luna-ended   - Emitted when playback ends. detail: { src, group }
 * @event luna-timeupdate - Emitted on timeupdate. detail: { currentTime, duration, progress }
 */
class LunaAudioPlayer extends HTMLElement {
  ...
  constructor() {
    ...
    this._dragTarget = null; // 'progress' | 'wave'

    this._barHeights = Array.from({ length: 60 }, (_, i) =>
  ...
  connectedCallback() {
    ...
    document.addEventListener('luna:audio:play', this._onGroupPlay);
  ...
  disconnectedCallback() {
    this._audio.pause();
    document.removeEventListener('luna:audio:play', this._onGroupPlay);
  ...
  _bindAudioEvents() {
    ...
    audio.addEventListener('play', () => {
      ...
      document.dispatchEvent(new CustomEvent('luna:audio:play', {
        detail: { player: this, group }
      }));
      this.dispatchEvent(new CustomEvent('luna-play', {
    ...
    audio.addEventListener('pause', () => {
      ...
      this.dispatchEvent(new CustomEvent('luna-pause', {
        bubbles: true, composed: true,
    ...
    audio.addEventListener('ended', () => {
      ...
      this._updatePlayButton(true);
      this.dispatchEvent(new CustomEvent('luna-ended', {
        bubbles: true, composed: true,
        detail:  { src: this.src, group: this._group() }
      }));
    });
    ...
    audio.addEventListener('timeupdate', () => {
      ...
      this.dispatchEvent(new CustomEvent('luna-timeupdate', {
        bubbles: true, composed: true,
    ...
    audio.addEventListener('durationchange', () => {
      ...
      if (el) {
        el.textContent = this._fmt(audio.duration);
      }
    });
  ...
  _updateProgress() {
    ...
    if (waveWrap)      { waveWrap.style.setProperty('--luna-audio-progress', `${pct}%`); }
  ...
  _resetProgress() {
    ...
    if (waveWrap)      { waveWrap.style.setProperty('--luna-audio-progress', '0%'); }
  ...
  _updateVolumeIcon() {
    ...
    if (slider) {
      slider.value = this._audio.muted ? 0 : this._audio.volume;
    }
  }

  _render() {
    const track  = this.getAttribute('track')  || 'Untitled';
    const artist = this.getAttribute('artist') || '';

    const bars = this._barHeights.map(h =>
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          display: block;
          width: 100%;
          max-width: var(--luna-audio-width, 480px);
          font-family: inherit;

          --luna-audio-bg:              #111;
          --luna-audio-fg:              #f0f0f0;
          --luna-audio-accent:          #ff5500;
          --luna-audio-muted:           #555;
          --luna-audio-waveform:        #2a2a2a;
          --luna-audio-waveform-played: var(--luna-audio-accent);
          --luna-audio-radius:          12px;
          --luna-audio-width:           480px;
          --luna-audio-padding:         20px 22px 18px;
          --luna-audio-progress:        0%;
        }
        ...
        *, *::before, *::after {
        ...
        .player {
          background: var(--luna-audio-bg);
          border-radius: var(--luna-audio-radius);
          padding: var(--luna-audio-padding);
          width: 100%;
          user-select: none;
        }
        ...
        .bar {
          flex: 1;
          background: var(--luna-audio-waveform);
          border-radius: 2px;
          min-width: 3px;
        ...
        .bar.played {
          background: var(--luna-audio-waveform-played);
        }
        ...
        .waveform-wrap::after {
          ...
          background: linear-gradient(
            90deg,
            transparent var(--luna-audio-progress),
            color-mix(in srgb, var(--luna-audio-accent) 6%, transparent) var(--luna-audio-progress)
          );
          pointer-events: none;
        }
        ...
        .play-btn {
          ...
          height: 36px;
          border-radius: 50%;
          background: var(--luna-audio-accent);
          color: #fff;
          cursor: pointer;
          display: flex;
          align-items: center;
...
customElements.define('luna-audio-player', LunaAudioPlayer);