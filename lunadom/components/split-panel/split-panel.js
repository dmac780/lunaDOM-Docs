 ...
 * @cssprop --luna-sp-divider-width        - Width (or height if vertical) of the divider bar. Default: 4px.
 * @cssprop --luna-sp-divider-hit-area    - Invisible pointer target around the bar. Default: 16px.
 * @cssprop --luna-sp-divider-color       - Resting bar colour. Default: var(--luna-border, #222).
 * @cssprop --luna-sp-divider-color-hover - Bar colour on hover / drag. Default: var(--luna-accent, #2563eb).
 * @cssprop --luna-sp-handle-size         - Length of the grip handle. Default: 28px.
 * @cssprop --luna-sp-handle-color        - Grip dot colour. Default: #555.
 * @cssprop --luna-sp-handle-color-hover  - Grip dot colour on hover / drag. Default: #aaa.
 * @cssprop --luna-sp-panel-bg            - Background applied to both panels. Default: transparent.
 *
 * Events:
 * @event luna-reposition - Fired while dragging. detail: { position: string, value: number }
 */
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --luna-sp-divider-width:        4px;
          --luna-sp-divider-hit-area:     16px;
          --luna-sp-divider-color:        var(--luna-border, #222);
          --luna-sp-divider-color-hover:  var(--luna-accent, #2563eb);
          --luna-sp-handle-size:          28px;
          --luna-sp-handle-color:         #555;
          --luna-sp-handle-color-hover:   #aaa;
          --luna-sp-panel-bg:             transparent;
        }
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .panel {
          position: relative;
          overflow: hidden;
          background: var(--luna-sp-panel-bg);
          min-width: 0;
          min-height: 0;
        }
        ...
        #divider {
          flex: 0 0 var(--luna-sp-divider-width);
          background: var(--luna-sp-divider-color);
          position: relative;
        ...
        #divider:hover,
        #divider.dragging {
          background: var(--luna-sp-divider-color-hover);
        }

        #divider:focus-visible {
          background: var(--luna-sp-divider-color-hover);
          box-shadow: 0 0 0 2px var(--luna-sp-divider-color-hover);
        }
        ...
        #divider::after {
          ...
          position: absolute;
          ${vertical
            ? 'left:0; top:50%; width:100%; height:var(--luna-sp-divider-hit-area); transform:translateY(-50%);'
            : 'top:0; left:50%; height:100%; width:var(--luna-sp-divider-hit-area); transform:translateX(-50%);'
          }
        }
        ...
        .handle {
          ...
          z-index: 11;
          ${vertical
            ? `width: var(--luna-sp-handle-size); height: 8px;`
            : `height: var(--luna-sp-handle-size); width: 8px;`
          }
        }
        ...
        .dot {
          ...
          height: 3px;
          border-radius: 50%;
          background: var(--luna-sp-handle-color);
          transition: background 0.15s ease;
        }

        #divider:hover .dot,
        #divider.dragging .dot {
          background: var(--luna-sp-handle-color-hover);
        }
