// lunadom/components/include/include.js

/**
 * @customElement luna-include
 * 
 ...
 * @cssprop --luna-accent - Color of the default loading spinner.
 * 
 * Events:
 * @event luna-load-start - Emitted when a load operation begins.
 * @event luna-load - Emitted when the content has successfully loaded.
 * @event luna-error - Emitted when the content fails to load.
 */
class LunaInclude extends HTMLElement {
  ...
  async load() {
    ...
    this.dispatchEvent(new CustomEvent('luna-load-start', {
      bubbles: true,
      composed: true
    }));
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('luna-load', {
        bubbles: true,
        composed: true
      }));
    } catch (err) {
      ...
      this.dispatchEvent(new CustomEvent('luna-error', {
        bubbles: true,
        composed: true,
        detail: { error: err }
      }));
  ...
  render(html = '') {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .content {
          display: ${this._isLoading ? 'none' : 'block'};
...
customElements.define('luna-include', LunaInclude);