// lunadom/components/tree-item/tree-item.js

/**
 * @customElement luna-tree-item
 * 
 ...
 * @cssprop --luna-tree-item-height - The height of the tree item.
 *
 * Events:
 * @event luna-expand - Emitted when the tree item is expanded.
 * @event luna-collapse - Emitted when the tree item is collapsed.
 * @event luna-select - Emitted when the tree item is selected.
 */
class LunaTreeItem extends HTMLElement {
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    if (this._isRendered && oldVal !== newVal) {
      ...
      if (name === 'expanded') {
        ...
        this.dispatchEvent(new CustomEvent(isExpanded ? 'luna-expand' : 'luna-collapse', { 
          bubbles: true, 
          composed: true 
        }));
      }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .item-row {
          display: flex;
          align-items: center;
          height: var(--luna-tree-item-height);
          padding: 0 0.5rem;
          cursor: pointer;
          user-select: none;
          border-radius: 4px;
        ...
        :host([selected]) .item-row {
          background: var(--luna-accent-alpha, rgba(37, 99, 235, 0.2));
      ...
      <div class="item-container">
        <div class="item-row" id="row" part="base">
          ...
          <div class="loading-container" id="loading-ui" hidden>
            <luna-spinner class="loading-spinner"></luna-spinner>
          </div>
  ...
  _syncState() {
    ...
    // An item is a leaf if it has no children AND isn't lazy
  ...
  _handleClick(e) {
    ...
    // Toggle logic: Expand if toggle clicked, or if label clicked on a non-leaf node
...
customElements.define('luna-tree-item', LunaTreeItem);