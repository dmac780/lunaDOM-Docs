// lunadom/components/bar-graph/bar-graph.js

/**
 * @customElement luna-bar-graph
 * 
 ...
 * @cssprop --luna-bar-graph-bg - Background color of the graph. Defaults to transparent.
 * @cssprop --luna-bar-graph-bar-color - Primary bar fill color. Defaults to #3b82f6.
 * @cssprop --luna-bar-graph-bar-negative-color - Color for negative value bars. Defaults to #ef4444.
 * @cssprop --luna-bar-graph-bar-hover-color - Bar color on hover. Defaults to #2563eb.
 * @cssprop --luna-bar-graph-bar-radius - Border radius for bar tips. Defaults to 3px.
 * @cssprop --luna-bar-graph-series-colors - Comma-separated color list for multi-series (overrides default palette).
 * @cssprop --luna-bar-graph-grid-color - Grid line color. Defaults to rgba(255,255,255,0.05).
 * @cssprop --luna-bar-graph-axis-color - Axis line color. Defaults to rgba(255,255,255,0.1).
 * @cssprop --luna-bar-graph-text-color - Axis label and tick text color. Defaults to inherit.
 * @cssprop --luna-bar-graph-value-label-color - Value label text color on bars. Defaults to inherit.
 * @cssprop --luna-bar-graph-goal-color - Goal line color. Defaults to #f59e0b.
 * @cssprop --luna-bar-graph-goal-dash - SVG stroke-dasharray for goal line. Defaults to '5,4'.
 * @cssprop --luna-bar-graph-transition - Animation duration. Defaults to 0.35s.
 * @cssprop --luna-bar-graph-legend-gap - Gap between legend items. Defaults to 1rem.
 * 
 * Events:
 * @event luna-bar-click - Emitted when a bar is clicked. Detail: { label, value, series, index }.
 * @event luna-bar-hover - Emitted when a bar is hovered or unhovered. Detail: { label, value, series, index } or null.
 */
class LunaBarGraph extends HTMLElement {
  ...
  _palette() {
    const custom = this._cssVar('--luna-bar-graph-series-colors');
    if (custom) {
      return custom.split(',').map(s => s.trim());
    }
    return [
      this._cssVar('--luna-bar-graph-bar-color', '#3b82f6'),
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          font-family: inherit;
          font-size: inherit;
          color: var(--luna-bar-graph-text-color, #aaa);
          background: var(--luna-bar-graph-bg, transparent);
          --luna-bar-graph-bar-color: #3b82f6;
          --luna-bar-graph-bar-negative-color: #ef4444;
          --luna-bar-graph-bar-hover-color: #2563eb;
          --luna-bar-graph-bar-radius: 3px;
          --luna-bar-graph-grid-color: rgba(255,255,255,0.05);
          --luna-bar-graph-axis-color: rgba(255,255,255,0.1);
          --luna-bar-graph-text-color: #aaa;
          --luna-bar-graph-value-label-color: #eee;
          --luna-bar-graph-goal-color: #f59e0b;
          --luna-bar-graph-goal-dash: 5,4;
          --luna-bar-graph-transition: 0.35s cubic-bezier(.4,0,.2,1);
          --luna-bar-graph-legend-gap: 1rem;
          position: relative;
        ...
        *, *::before, *::after {
        ...
        .legend {
          display: flex;
          flex-wrap: wrap;
          gap: 0.25rem var(--luna-bar-graph-legend-gap);
          font-size: 0.78em;
          opacity: 0.85;
          padding: 0 4px;
        }
        ...
        .bar {
          transition: opacity var(--luna-bar-graph-transition);
          cursor: pointer;
        }
        ...
        .value-label {
          font-size: 0.68em;
          fill: var(--luna-bar-graph-value-label-color, #eee);
          pointer-events: none;
          font-weight: 500;
        }
        ...
        .goal-label-text {
          font-size: 0.68em;
          fill: var(--luna-bar-graph-goal-color, #f59e0b);
        }
      </style>
      <div class="root">
  ...
  _draw() {
    ...
    const negativeColor = this._cssVar('--luna-bar-graph-bar-negative-color', '#ef4444');
    const barRadius = this._cssVar('--luna-bar-graph-bar-radius', '3px');
    ...
    const gridColor = this._cssVar('--luna-bar-graph-grid-color', 'rgba(255,255,255,0.05)');
    const axisColor = this._cssVar('--luna-bar-graph-axis-color', 'rgba(255,255,255,0.1)');
    ...
    if (!isNaN(goalVal)) {
      const gc = this._cssVar('--luna-bar-graph-goal-color', '#f59e0b');
      const gd = this._cssVar('--luna-bar-graph-goal-dash', '5,4');
    ...
    barEvents.forEach(({ id, label, value, series, index }) => {
      ...
      el.addEventListener('mouseenter', (e) => {
        ...
        this.dispatchEvent(new CustomEvent('luna-bar-hover', {
      ...
      el.addEventListener('click', () => {
        this.dispatchEvent(new CustomEvent('luna-bar-click', {
          detail: { label, value, series, index },
          bubbles: true,
          composed: true
        }));
...
customElements.define('luna-bar-graph', LunaBarGraph);
