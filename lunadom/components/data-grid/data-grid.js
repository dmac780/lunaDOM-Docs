// lunadom/components/data-grid/data-grid.js

/**
 * @customElement luna-data-grid
 *
 * Attributes:
 ...
 * @cssprop --luna-grid-bg              - Card background (default: #1a1a1a)
 * @cssprop --luna-grid-border          - Card / cell border colour (default: #222)
 * @cssprop --luna-grid-radius          - Card border radius (default: 1rem)
 * @cssprop --luna-grid-header-bg       - Header row background (default: #111)
 * @cssprop --luna-grid-header-color    - Header text colour (default: #555)
 * @cssprop --luna-grid-header-size     - Header font size (default: 0.6875rem)
 * @cssprop --luna-grid-row-bg          - Row background (default: transparent)
 * @cssprop --luna-grid-row-hover       - Row hover background (default: rgba(255,255,255,.03))
 * @cssprop --luna-grid-row-selected    - Selected row background (default: rgba(37,99,235,.1))
 * @cssprop --luna-grid-row-stripe      - Stripe row tint (default: rgba(255,255,255,.015))
 * @cssprop --luna-grid-cell-color      - Cell text colour (default: #ccc)
 * @cssprop --luna-grid-cell-size       - Cell font size (default: 0.8125rem)
 * @cssprop --luna-grid-cell-padding    - Cell padding (default: 0.625rem 1rem)
 * @cssprop --luna-grid-accent          - Sort arrow / checkbox accent (default: #2563eb)
 * @cssprop --luna-grid-divider         - Divider between header and body (default: #222)
 * @cssprop --luna-grid-pagination-color - Pagination text colour (default: #555)
 * @cssprop --luna-grid-scrollbar-color  - Scrollbar thumb colour (default: #2a2a2a)
 *
 * Properties (JS only):
 ...
 *
 * Events:
 * @event luna-sort     - Fired when the sort column or direction changes.
 *   detail: { key: string, direction: 'asc'|'desc' }
 * @event luna-filter   - Fired when the filter query changes.
 *   detail: { query: string, count: number }
 * @event luna-select   - Fired when row selection changes.
 *   detail: { selected: number[], rows: object[] }
 * @event luna-page     - Fired when the active page changes.
 *   detail: { page: number, pageSize: number, total: number }
 * @event luna-row-click - Fired when a row is clicked (not on an interactive child).
 ...
 * 
 */
class LunaDataGrid extends HTMLElement {
  ...
  /**
   * Wire a <luna-input> (or any element that emits input events) to this
  ...
  _onFilterInput(e) {
    ...
    this.dispatchEvent(new CustomEvent('luna-filter', {
  ...
  _toggleSort(key) {
    ...
    this.dispatchEvent(new CustomEvent('luna-sort', {
      bubbles: true, composed: true,
  ...
  _emitSelect() {
    ...
    this.dispatchEvent(new CustomEvent('luna-select', {
      bubbles: true, composed: true,
      detail: {
        selected: idxs,
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --luna-grid-bg:               #1a1a1a;
          --luna-grid-border:           #222;
          --luna-grid-radius:           1rem;
          --luna-grid-header-bg:        #111;
          --luna-grid-header-color:     #555;
          --luna-grid-header-size:      0.6875rem;
          --luna-grid-row-bg:           transparent;
          --luna-grid-row-hover:        rgba(255, 255, 255, 0.03);
          --luna-grid-row-selected:     rgba(37, 99, 235, 0.08);
          --luna-grid-row-stripe:       rgba(255, 255, 255, 0.015);
          --luna-grid-cell-color:       #ccc;
          --luna-grid-cell-size:        0.8125rem;
          --luna-grid-cell-padding:     0.625rem 1rem;
          --luna-grid-accent:           #2563eb;
          --luna-grid-divider:          #1e1e1e;
          --luna-grid-pagination-color: #555;
          --luna-grid-scrollbar-color:  #2a2a2a;
        }

        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

        .shell {
          background:    ${noBorder ? 'transparent' : 'var(--luna-grid-bg)'};
          border:        ${noBorder ? 'none'        : '1px solid var(--luna-grid-border)'};
          border-radius: ${noBorder ? '0'           : 'var(--luna-grid-radius)'};
          overflow: hidden;
          display: flex;
        ...
        .scroll-wrap {
          ...
          scrollbar-color: var(--luna-grid-scrollbar-color) transparent;
        }

        .scroll-wrap::-webkit-scrollbar        { height: 5px; }
        .scroll-wrap::-webkit-scrollbar-track  { background: transparent; }
        .scroll-wrap::-webkit-scrollbar-thumb  { background: var(--luna-grid-scrollbar-color); border-radius: 3px; }
        ...
        thead { background: var(--luna-grid-header-bg); }

        thead tr {
          border-bottom: 1px solid var(--luna-grid-divider);
        }

        .th {
          padding: var(--luna-grid-cell-padding);
          font-size: var(--luna-grid-header-size);
          font-weight: 600;
          color: var(--luna-grid-header-color);
        ...
        .sort-icon {
          width: 9px;
          height: 9px;
          fill: var(--luna-grid-accent);
          flex-shrink: 0;
        }

        .sort-icon.idle { fill: var(--luna-grid-header-color); }

        tbody tr.tr {
          background:  var(--luna-grid-row-bg);
          border-bottom: 1px solid var(--luna-grid-divider);
        ...
        tbody tr.tr:hover  { background: var(--luna-grid-row-hover); }
        tbody tr.selected  { background: var(--luna-grid-row-selected) !important; }
        tbody tr.stripe    { background: var(--luna-grid-row-stripe); }

        .td {
          padding: var(--luna-grid-cell-padding);
          font-size: var(--luna-grid-cell-size);
          color: var(--luna-grid-cell-color);
          white-space: nowrap;
          overflow: hidden;
        ...
        .check-wrap input:checked  ~ .cb-box {
          background:    var(--luna-grid-accent);
          border-color:  var(--luna-grid-accent);
        }
        ...
        /* ── skeleton ── */
        .skeleton-row td { border-bottom: 1px solid var(--luna-grid-divider); }
        ...
        .pagination {
          ...
          border-top: 1px solid var(--luna-grid-divider);
          gap: 0.5rem;
        }

        .pg-info {
          font-size: 0.6875rem;
          color: var(--luna-grid-pagination-color);
          white-space: nowrap;
        }
  ...
  _bindEvents(pageItems, visibleOrigIdxs, selectable) {
    ...
    root.querySelectorAll('tr[data-orig-idx]').forEach(tr => {
      tr.addEventListener('click', e => {
        if (e.target.closest('input, button, a, luna-button')) {
          return;
        }

        const origIdx = parseInt(tr.dataset.origIdx, 10);
        this.dispatchEvent(new CustomEvent('luna-row-click', {
          bubbles: true, composed: true,
    ...
    root.querySelectorAll('.pg-btn[data-page]').forEach(btn => {
      ...
      btn.addEventListener('click', () => {
        ...
        this.dispatchEvent(new CustomEvent('luna-page', {
          bubbles: true, composed: true,
...
customElements.define('luna-data-grid', LunaDataGrid);