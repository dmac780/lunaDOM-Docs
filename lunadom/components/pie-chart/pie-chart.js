// lunadom/components/pie-chart/pie-chart.js

/**
 * @customElement luna-pie-chart
 *
 ...
 *   The optional `color` field accepts any CSS colour string and takes
 *   precedence over the palette for that slice.
 *
 * @attr {string}  title
 ...
 *   Static label shown beneath the centre value in donut mode.
 *   Defaults to "total".
 *
 * @attr {boolean} no-bg
 *   Removes the card background, border-radius and padding entirely.
 *   Use this when embedding the chart inside a card or other surface.
 *
 ...
 * @cssprop --luna-pie-size          - Diameter of the chart (default: 180px)
 * @cssprop --luna-pie-bg            - Card background (default: #1a1a1a)
 * @cssprop --luna-pie-border        - Card border colour (default: #222)
 * @cssprop --luna-pie-fg            - Primary text colour (default: #eee)
 * @cssprop --luna-pie-muted         - Secondary / label colour (default: #555)
 * @cssprop --luna-pie-radius        - Card border radius (default: 1rem)
 * @cssprop --luna-pie-padding       - Card inner padding (default: 1.25rem)
 * @cssprop --luna-pie-gap           - Gap between chart and legend (default: 1rem)
 * @cssprop --luna-pie-tooltip-bg    - Tooltip background (default: #1e1e1e)
 * @cssprop --luna-pie-tooltip-fg    - Tooltip text colour (default: #eee)
 * @cssprop --luna-pie-palette       - Comma-separated fallback colour list used
 *                                     when a data item has no `color` field.
 *                                     Overrides the built-in default palette.
 *                                     e.g. --luna-pie-palette: #f00, #0f0, #00f;
 *
 * Events:
 * @event luna-slice-enter - Fired when a slice is hovered/focused.
 *   detail: { index, label, value, percent, color }
 * @event luna-slice-leave - Fired when hover/focus leaves a slice.
 */
class LunaPieChart extends HTMLElement {
  ...
  _resolvePalette() {
    const raw = getComputedStyle(this).getPropertyValue('--luna-pie-palette').trim();

    if (raw) {
      return raw.split(',').map(s => s.trim()).filter(Boolean);
    }

    return LunaPieChart._defaultPalette;
  }
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --luna-pie-fg:         #eee;
          --luna-pie-muted:      #555;
          --luna-pie-radius:     1rem;
          --luna-pie-padding:    1.25rem;
          --luna-pie-gap:        1rem;
          --luna-pie-tooltip-bg: #1e1e1e;
          --luna-pie-tooltip-fg: #eee;
        }
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card {
          ...
          gap: var(--luna-pie-gap);
          background: ${noBg ? 'transparent' : 'var(--luna-pie-bg)'};
          border: ${noBg ? 'none' : '1px solid var(--luna-pie-border)'};
          border-radius: ${noBg ? '0' : 'var(--luna-pie-radius)'};
          padding: ${noBg ? '0' : 'var(--luna-pie-padding)'};
        }

        .title {
          font-size: 0.8125rem;
          font-weight: 600;
          color: var(--luna-pie-fg);
          letter-spacing: -0.01em;
        }

        .wrap {
          position: relative;
          width: var(--luna-pie-size);
          height: var(--luna-pie-size);
          align-self: center;
        }
        ...
        .tooltip {
          position: fixed;
          background: var(--luna-pie-tooltip-bg);
          color: var(--luna-pie-tooltip-fg);
          font-size: 0.6875rem;
          padding: 6px 11px;
          border-radius: 6px;
          pointer-events: none;
  ...
  _bindEvents(slices, total, isDonut, donutLabel) {
    ...
    const showSlice = (e, s) => {
      ...
      this.dispatchEvent(new CustomEvent('luna-slice-enter', {
        bubbles:  true,
        composed: true,
        detail: {
          index:   s.index,
    ...
    const hideSlice = () => {
      ...
      this.dispatchEvent(new CustomEvent('luna-slice-leave', {
...
customElements.define('luna-pie-chart', LunaPieChart);