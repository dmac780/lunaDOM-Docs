// lunadom/components/drawer/drawer.js

/**
 * @customElement luna-drawer
 * 
 ...
 * @cssprop --luna-drawer-size - The width (for start/end) or height (for top/bottom) of the drawer panel.
 * @cssprop --luna-bg - Background color of the drawer panel.
 * @cssprop --luna-color - Text color within the drawer.
 * @cssprop --luna-border - Border color for segments and edges.
 * @cssprop --luna-overlay-bg - Background color of the backdrop overlay.
 * @cssprop --luna-transition-speed - Animation duration for slide and fade.
 * 
 * Events:
 * @event luna-show - Emitted when the drawer opens.
 * @event luna-hide - Emitted when the drawer closes.
 * @event luna-request-close - Emitted when the drawer attempts to close (via close button, ESC, or overlay). Can be cancelled.
 */
class LunaDrawer extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (this._isRendered && (name === 'placement' || name === 'contained')) {
      // Re-render styles/structure if placement or contained mode changes
      this.render();
    }
  }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          --luna-drawer-size: 300px;
          --luna-bg: #fff;
          --luna-color: #111;
          --luna-border: #e5e7eb;
          --luna-overlay-bg: rgba(0, 0, 0, 0.4);
          --luna-transition-speed: 0.3s;
          ...
          display: flex;
          visibility: hidden;
          transition: visibility var(--luna-transition-speed);
          pointer-events: none;
          overflow: hidden;
        }
        ...
        .overlay {
          position: absolute;
          inset: 0;
          background: var(--luna-overlay-bg);
          backdrop-filter: blur(4px);
          opacity: 0;
          transition: opacity var(--luna-transition-speed);
        ...
        .panel {
          ...
          transition: transform var(--luna-transition-speed) cubic-bezier(0.4, 0, 0.2, 1);
          overflow: hidden;
...
customElements.define('luna-drawer', LunaDrawer);